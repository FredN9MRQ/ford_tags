#!/bin/bash
# Script to update configuration.yaml on Home Assistant server
# Run this from the HA console after typing 'login'
...
echo "Writing new configuration.yaml..."
cat > configuration.yaml <<'ENDOFCONFIG'
...
echo "File backed up to: configuration.yaml.backup_$(date +%Y%m%d_%H%M%S)"
echo ""
echo "Next steps:"
echo "1. Exit back to ha > prompt with: exit"
echo "2. Check config with: ha core check"
echo "3. Restart HA with: ha core restart"
